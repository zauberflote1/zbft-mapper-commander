...
int rc_tf_ringbuf_get_tf_at_time(rc_tfv_ringbuf_t* buf, int64_t ts, rc_tf_t* result)
{
	...
	// allow timestamps up to 0.1s newer than our last position record
	if(ts > (buf->latest_ts+100000000)){
		//fprintf(stderr,"ERROR in %s, requested ts %5.1fms newer than our most recent data\n", __FUNCTION__, (double)(ts-buf->latest_ts)/1000000.0);
		pthread_mutex_unlock(&buf->mutex);
		return -3;
	}
